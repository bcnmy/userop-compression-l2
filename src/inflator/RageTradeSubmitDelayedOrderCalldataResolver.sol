...
contract RageTradeSubmitDelayedOrderCalldataResolver is IInflator {
    ...
    function inflate(bytes calldata _data) external view override returns (bytes memory userOpCalldata) {
    ...
    function deflate(bytes calldata _data) external pure returns (bytes memory compressedData) {
